...
(function () {
    ...
    function selectCollaboratorFromResults(rowIndex = 1) {
        ...
        for (const item of userItems) {
            ...
            if (userName) {
                ...
                // 根据匹配模式进行比较
                const isMatch = userName === CONFIG.collaboratorKeyword;
        ...
        if (matchedUsers.length > 0) {
            ...
            const hasPartialMatch = Array.from(userItems).some(item => {
                ...
                return text.includes(CONFIG.collaboratorKeyword);
            });

            if (hasPartialMatch) {
                showNotification(`⚠️ 找到类似人员但未精确匹配 "${CONFIG.collaboratorKeyword}"，请手动选择`, 'warning');
            } else {
                showNotification(`⚠️ 未找到共同完成人 "${CONFIG.collaboratorKeyword}"`, 'warning');
            }
        }
    ...
    function selectProjectFromSearchResults(rowIndex, projectDialogElement) {
        ...
        for (let i = 0; i < projectRows.length; i++) {
            ...
            // 简化为只按名称精确匹配
            if (rowProjectName === CONFIG.projectKeyword) {
                isMatch = true;
            } else if (rowProjectName.includes(CONFIG.projectKeyword) || CONFIG.projectKeyword.includes(rowProjectName)) {
                // 记录部分匹配，用于提示，但不在 data-field="code" 模式下
                console.log(`发现部分匹配项目: ${rowProjectName} (关键词: ${CONFIG.projectKeyword})`);
                // 只有当没有找到完全匹配时，才可能需要用到这个信息
                // 这里我们暂时不处理，因为要求是必须精确匹配
            }
