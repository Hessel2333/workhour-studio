...
(function () {
    ...
    const DEFAULT_CONFIG = {
        name: '默认配置',
        workNature: 'workCategory_fky', // 默认：事务性工作
        workCategory: 'workCategory_fky1', // 默认：日常事务
        workForm: '14', // 默认：其他
        ...
        collaboratorKeyword: "" // 共同完成人关键词
    };

    // 配置管理器
    // 默认全局偏好
    const DEFAULT_GLOBAL_PREFS = {
        workDateMode: "today", // today, yesterday, specific
        specificWorkDate: ""
    };

    class GlobalPreferenceManager {
        constructor() {
        ...
        loadPrefs() {
            try {
                const stored = localStorage.getItem('workhour_global_prefs');
                return stored ? { ...DEFAULT_GLOBAL_PREFS, ...JSON.parse(stored) } : { ...DEFAULT_GLOBAL_PREFS };
            } catch (e) {
                console.error('读取全局偏好失败:', e);
        ...
        savePrefs(newPrefs) {
            try {
                ...
                localStorage.setItem('workhour_global_prefs', JSON.stringify(this.prefs));
                console.log('全局偏好已保存:', this.prefs);
                return true;
            } catch (e) {
                console.error('保存全局偏好失败:', e);
                return false;
            }
        }
    ...
    // 全局配置管理器实例
    let configManager;
    let globalPrefs; // 新增全局偏好设置
    let CONFIG;

    try {
        configManager = new ConfigManager();
        globalPrefs = new GlobalPreferenceManager(); // 初始化全局设置
        ...
        globalPrefs = new GlobalPreferenceManager(); // 即使失败也尝试初始化全局设置
    ...
    // 处理关联项目
    function handleProjectSelection(rowIndex = 1, projectKeyword = null) {
        // 如果没有传递 projectKeyword，则尝试从 CONFIG 获取
        const keyword = projectKeyword || CONFIG.projectKeyword;

        // 获取实际的ID后缀
        const actualIdSuffix = getActualRowIdSuffix(rowIndex);

        if (!keyword) {
            console.log(`ℹ️ 第 ${rowIndex} 行未配置项目关键词，跳过自动选择`);
            return;
        }
        ...
        if (!keyword) {
            ...
            // 情况1：如果是【探索项目】，则直接跳过，不提示
            if (CONFIG.workCategory === 'ky_exploration') {
                console.log(`ℹ️ 当前为【探索项目】且未配置项目名称，跳过关联项目选择`);
                return;
            }

            // 情况2：其他情况，如果没有填项目名称，显示手动选择提示
            console.log(`⚠️ 第 ${rowIndex} 行关联项目需要填写项目名称，当前配置为空`);
    ...
    function searchAndSelectProject(rowIndex, projectDialog = document) {
        const keyword = window.currentProjectKeyword || CONFIG.projectKeyword;
    ...
    function fillWorkTimeRow(rowIndex = 1, sourceData = null) {
        console.log(`开始填写第 ${rowIndex} 行的工时内容${sourceData ? ' (使用Excel数据)' : ' (使用配置数据)'}`);
        ...
        // 决定使用的数据来源 (Excel数据 覆盖 配置数据)
        const data = {
            workNature: sourceData ? findValueByText(sourceData['工作性质'], 'nature') : CONFIG.workNature,
            // category 由 handleWorkCategory 处理，但这里也可以准备好
            workFormText: sourceData ? sourceData['工作形式'] : null, // Excel提供文本
            workForm: CONFIG.workForm, // 配置提供Value
            startTime: sourceData ? sourceData['开始时间'] : CONFIG.defaultStartTime,
            endTime: sourceData ? sourceData['结束时间'] : CONFIG.defaultEndTime,
            remark: sourceData ? sourceData['备注'] : CONFIG.defaultRemark,
            project: sourceData ? sourceData['关联项目'] : CONFIG.projectKeyword,
            collaborator: sourceData ? sourceData['共同完成人'] : CONFIG.collaboratorKeyword
        };
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                // 第三步：工作形式 - 根据配置选择
                // 如果是Excel数据，尝试通过文本匹配Value
                if (sourceData && data.workFormText) {
                    ...
                    if (formSelect) {
                        ...
                        else console.warn(`未找到工作形式 "${data.workFormText}" 对应的选项`);
                    }
                } else {
                ...
                setTimeout(() => {
                    ...
                    if (startTimeElement && startTimeElement.tagName === 'SELECT') {
                        // 新的select结构，直接使用时间格式
                        fillFormField(baseSelectors.startTime, data.startTime, true); // true表示是select
                    ...
                    // 第五步：处理关联项目
                    handleProjectSelection(rowIndex, data.project);
    ...
    // 主要的自动填写函数
    async function autoFillWorkTime() {
        try {
            // 显示开始提示
            // 显示开始提示
            // 0. 检查已启用配置数量
            const enabledCount = configManager.configs.filter(c => c.enabled).length;
            if (enabledCount > 1) {
                alert('⚠️ 检测到多个已启用的配置！\n\n为避免混淆，请在设置中只保留一个勾选的配置，然后重试。');
                return;
            }
            ...
            // 0. 首先检查并设置工时日期 (全局配置)
            // 只有当模式不是 today 时才需要设置
            if (globalPrefs && globalPrefs.current.workDateMode && globalPrefs.current.workDateMode !== 'today') {
                showNotification('📅 正在应用全局日期设置...', 'info');
                await setWorkDate(globalPrefs.current.workDateMode, globalPrefs.current.specificWorkDate);
                // 等待日期设置这可能引发的页面刷新或变化
                await new Promise(r => setTimeout(r, 500));
            }

            // 填写工作日期
            // if (CONFIG.autoFillCurrentDate) { // autoFillCurrentDate 已移除
